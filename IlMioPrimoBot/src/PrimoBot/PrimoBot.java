...
public class PrimoBot extends Bot {
	...
	public void textMessage(Message message) {
		...
		ComandoPrimoBot c = ComandoPrimoBot.fromString(message.getText());
		...
		switch (c) {
			...
			case TASTIERA:
				...
				line.add(new KeyboardButton(ComandoPrimoBot.MONOPATTINO.toString()));
				line.add(new KeyboardButton(ComandoPrimoBot.SCOOTER.toString()));
				line.add(new KeyboardButton(ComandoPrimoBot.MOTO.toString()));
				keyboard.add(line);
			...
			case START:
				...
				break;
			case ERRORE:
				messageToSend = new MessageToSend(message.getChat().getId(), "PrimoBot.Comando non valido");
				sendMessage(messageToSend);
				break;
		}
	}
